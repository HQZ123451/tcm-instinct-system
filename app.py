...
def show_multimodal_diagnosis():
    st.header("🔬 本能系统多模态诊断")
    st.markdown("*望（舌象、体象）+ 问（症状、问诊），AI综合《生命本能系统论》进行智能分析*")
    st.markdown("---")

    api_keys = get_api_keys()

    # ========== 望诊区域（舌象 + 体象）==========
    st.subheader("👁️ 望诊信息采集")
    tongue_col, body_col = st.columns([1, 1])

    with tongue_col:
        st.markdown("**📷 舌象采集（望舌）**")
        uploaded_file = st.file_uploader("请上传清晰的舌象照片", type=['jpg', 'jpeg', 'png'], key="tongue_upload")
        ...
        tongue_image = None
        if uploaded_file:
            tongue_image = preprocess_image(uploaded_file)
            st.image(tongue_image, caption="已上传舌象", use_column_width=True)

    with body_col:
        st.markdown("**📷 体象采集（望体态/面色）**")
        body_file = st.file_uploader("请上传体态或面色照片（可选）", type=['jpg', 'jpeg', 'png'], key="body_upload")
        ...
        body_image = None
        if body_file:
            body_image = preprocess_image(body_file)
            st.image(body_image, caption="已上传体象", use_column_width=True)

    st.markdown("---")

    # ========== 问诊区域（症状 + 详细问诊）==========
    st.subheader("🗣️ 问诊信息采集")
    symptom_tab, inquiry_tab = st.tabs(["📝 症状选择", "📋 详细问诊（问象）"])
    ...
    with symptom_tab:
        ...
        selected_symptoms = st.multiselect("选择主要症状", common_symptoms, key="symptom_select")
        custom_symptoms = st.text_input("或手动输入其他症状：", placeholder="例如：头疼 口渴 失眠", key="symptom_custom")
    ...
    with inquiry_tab:
        st.markdown("**详细问诊信息采集（中医十问）**")
        q1, q2 = st.columns(2)
        with q1:
            inquiry_info["睡眠"] = st.selectbox("睡眠质量", ["请选择", "良好", "入睡困难", "多梦易醒", "彻夜不眠"], key="sleep")
            inquiry_info["饮食"] = st.selectbox("饮食情况", ["请选择", "食欲正常", "食欲不振", "多食易饥", "口苦口干"], key="diet")
            inquiry_info["情绪"] = st.selectbox("情绪状态", ["请选择", "平和稳定", "焦虑烦躁", "抑郁低落", "易怒激动"], key="mood")
            inquiry_info["寒热"] = st.selectbox("寒热感觉", ["请选择", "正常", "畏寒怕冷", "发热怕热", "寒热往来"], key="temperature")
        with q2:
            inquiry_info["汗出"] = st.selectbox("出汗情况", ["请选择", "正常", "自汗", "盗汗", "无汗"], key="sweat")
            inquiry_info["二便"] = st.selectbox("二便情况", ["请选择", "大小便正常", "大便干结", "大便溏薄", "小便频数", "小便不利"], key="urine")
            inquiry_info["体力"] = st.selectbox("体力状况", ["请选择", "精力充沛", "容易疲劳", "四肢乏力", "气短懒言"], key="energy")
            inquiry_info["疼痛"] = st.text_input("疼痛部位（如有）", placeholder="例如：头痛、腹痛", key="pain")

        # 过滤未选择的选项
        inquiry_info = {k: v for k, v in inquiry_info.items() if v and v != "请选择"}
        if inquiry_info:
            st.success(f"已采集问诊信息：{len(inquiry_info)}项")

    st.markdown("---")
    analyze_col1, analyze_col2, analyze_col3 = st.columns([1, 2, 1])
    with analyze_col2:
        if st.button("🔍 开始本能系统多模态分析", type="primary", use_container_width=True):
            if not uploaded_file and not body_file:
                st.error("请至少上传舌象或体象照片之一")
            elif not api_keys.get("dashscope_key"):
                st.error("未配置通义千问API Key")
            else:
                with st.spinner("🤖 AI正在进行多模态综合分析..."):
                    result = full_multimodal_analysis(
                        tongue_image=tongue_image,
                        body_image=body_image,
                        symptoms=all_text_symptoms or None,
                        inquiry_info=inquiry_info if inquiry_info else None,
                        api_key=api_keys["dashscope_key"]
                    )
                    if result["success"]:
...
if __name__ == "__main__":
    ...
    if 'analysis_result' in st.session_state:
        ...
        if result.get("tongue_features"):
            ...
            st.caption("未上传舌象或未能识别舌象特征")

        st.markdown("**👤 体象诊察（望体态/面色）**")
        if result.get("body_features"):
            for feature in result["body_features"]:
                st.markdown(f"- ✅ {feature}")
        else:
            st.caption("未上传体象或未能识别体象特征")

        # 2. 问诊分析
        st.subheader("二、问诊分析（问象）")
        if result.get("inquiry_analysis"):
            for item in result["inquiry_analysis"]:
                st.markdown(f"- ✅ {item}")
        else:
            st.caption("未采集详细问诊信息")

        # 3. 本能系统分析
        st.subheader("三、本能系统分析")
